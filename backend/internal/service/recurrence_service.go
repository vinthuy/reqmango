...
func (s *RecurrenceService) Create(req *request.RecurrenceCreateRequest, issueID uint64) (*response.RecurrenceResponse, error) {
	...
	if count > 0 {
		return nil, common.Conflict("Issue already has a recurrence rule")
	}

	nextRun, _ := time.Parse(time.RFC3339, req.NextRun)
	if nextRun.IsZero() {
		nextRun = s.computeNext(req.Frequency, req.Interval, time.Now())
	}
	...
	if err := s.db.Create(r).Error; err != nil {
		return nil, common.Internal("Failed to create")
	}
...
func (s *RecurrenceService) Update(issueID uint64, req *request.RecurrenceUpdateRequest) (*response.RecurrenceResponse, error) {
	...
	if req.Frequency != nil {
		u["frequency"] = *req.Frequency
	}
	if req.Interval != nil {
		u["interval"] = *req.Interval
	}
	if req.IsActive != nil {
		u["is_active"] = *req.IsActive
	}
	if req.CronExpr != nil {
		u["cron_expr"] = req.CronExpr
	}
	if req.NextRun != nil {
...
func (s *RecurrenceService) Delete(issueID uint64) error {
	...
	if r.RowsAffected == 0 {
		return common.NotFound("No recurrence rule found")
	}
	return r.Error
}
