...
func NewAIService(db *gorm.DB, llmClient *llm.LLMClient, backend *client.BackendClient) *AIService {
	return &AIService{db: db, llm: llmClient, backend: backend}
}
...
// projectStatistics is an unexported copy of backend dto/response.ProjectStatistics.
...
// issuesSummary is an unexported copy of backend dto/response.IssuesSummary.
...
// getProjectStatistics is an inline replacement for the backend's projectSvc.GetStatistics.
func (s *AIService) getProjectStatistics(projectID uint64) (*projectStatistics, error) {
...
// getIssuesSummary is an inline replacement for the backend's projectSvc.GetIssuesSummary.
func (s *AIService) getIssuesSummary(projectID uint64) (*issuesSummary, error) {
	var project model.Project
	if err := s.db.First(&project, projectID).Error; err != nil {
		return nil, common.NotFound("Project not found")
	}
...
func (s *AIService) GenerateChart(ctx context.Context, req *AIChartRequest, actx *AIContext) (*AIChartResponse, error) {
	...
	stats, _ := s.getProjectStatistics(actx.ProjectID)
	summary, _ := s.getIssuesSummary(actx.ProjectID)
...
func (s *AIService) Analyze(ctx context.Context, actx *AIContext) (*AIAnalyzeResponse, error) {
	...
	stats, err := s.getProjectStatistics(actx.ProjectID)
	if err != nil {
		return nil, fmt.Errorf("get stats: %w", err)
	}
	summary, err := s.getIssuesSummary(actx.ProjectID)
	if err != nil {
...
func (s *AIService) toolGetProjectStats(args map[string]interface{}, actx *AIContext) (any, error) {
	...
	stats, err := s.getProjectStatistics(pid)
	if err != nil {
		return nil, err
	}
	return stats, nil
...
func (s *AIService) toolGetIssuesSummary(args map[string]interface{}, actx *AIContext) (any, error) {
	pid := getUintArg(args, "project_id", actx.ProjectID)
	summary, err := s.getIssuesSummary(pid)
	if err != nil {
		return nil, err
	}
	return summary, nil
