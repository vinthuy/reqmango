...
async function save() {
  if (!form.value.name.trim()) {
      alert(t('initiative.nameRequired'))
      return
    }
  if (editing.value) {
...
<template>
  <div class="p-6 max-w-6xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      ...
      <button @click="showForm = true; editing = null; form = { name: '', description: '', color: '#3b82f6', status: 'active', target_date: '', start_date: '', project_ids: [] }" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">{{ t('initiative.create') }}</button>
    </div>
