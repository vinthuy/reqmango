          ...
          <tr v-for="a in approvals" :key="a.id"
            ...
            <td class="px-4 py-2">
              <div class="flex gap-2">
                <router-link :to="`/workspace/${slug}/project/${a.project_id}/issues/${a.issue_id}`"
                  class="inline-flex items-center gap-1 px-3 py-1 text-xs font-medium text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded transition-colors"
                  :title="t('common.view') || '查看详情'">
                ...
                <template v-if="canDecide(a)">
                  <button @click="decide(a, 'approved')"
                    class="inline-flex items-center gap-1 px-3 py-1 text-xs font-medium text-white bg-green-600 hover:bg-green-700 rounded transition-colors"
                    :title="t('approvals.approveHint') || '批准此审批申请'">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                    {{ t('approvals.approve') }}
                  </button>
                  <button @click="decide(a, 'rejected')"
                    class="inline-flex items-center gap-1 px-3 py-1 text-xs font-medium text-white bg-red-600 hover:bg-red-700 rounded transition-colors"
                    :title="t('approvals.rejectHint') || '拒绝此审批申请'">
