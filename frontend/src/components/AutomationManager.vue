...
function openCreate() { form = ref({ name:'', desc:'', trigger:'issue_created', conditions:'[]', actions:'[]' }); showModal.value = true }
...
async function confirmDel(a:any) { if(confirm('Delete?')) { await workflowApi.deleteAutomation(props.projectId, a.id); load() } }
onMounted(load)
</script>
