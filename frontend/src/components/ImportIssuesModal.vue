<template>
  <div v-if="visible" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="handleClose">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col">
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
        <h2 class="text-lg font-semibold text-gray-800">批量导入工作项</h2>
      ...
      <div class="flex-1 overflow-y-auto p-6">
        ...
        <div v-if="mode === 'csv'" class="space-y-4">
          <div class="bg-blue-50 border border-blue-200 rounded-md p-4">
            <h4 class="text-sm font-medium text-blue-800 mb-2">CSV 格式说明</h4>
            <p class="text-xs text-blue-700 mb-2">支持以下列名（中英文均可）：</p>
          ...
          <div class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-indigo-400 transition-colors cursor-pointer"
            ...
            <div v-if="!selectedFile">
              ...
              <p class="text-sm text-gray-600 mb-2">点击或拖拽 CSV 文件到此处</p>
            ...
            <div v-else class="flex items-center justify-center space-x-3">
              ...
              <button @click.stop="clearFile" class="text-red-500 hover:text-red-700 text-sm ml-2">移除</button>
            </div>
          </div>
        </div>
        ...
        <div v-else class="space-y-4">
          <div class="bg-green-50 border border-green-200 rounded-md p-4">
            <h4 class="text-sm font-medium text-green-800 mb-2">JSON 格式说明</h4>
            <pre class="text-xs text-green-700 bg-green-100/50 p-3 rounded overflow-x-auto">[
  {
    ...
    "parent_title": "父工作项标题"
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">粘贴 JSON 数据</label>
            <textarea
              v-model="jsonText"
              rows="10"
        ...
        <div v-if="importResult" class="mt-6">
          <div :class="['rounded-md p-4', importResult.fail_count > 0 ? 'bg-yellow-50 border border-yellow-200' : 'bg-green-50 border border-green-200']">
            <div class="flex items-center justify-between mb-2">
              ...
              <div class="text-sm">
                <span class="text-green-600 font-medium">成功 {{ importResult.success_count }}</span>
                <span v-if="importResult.fail_count > 0" class="text-red-600 font-medium ml-3">失败 {{ importResult.fail_count }}</span>
              </div>
            </div>
            <div v-if="importResult.errors.length > 0" class="mt-3 max-h-40 overflow-y-auto">
              <ul class="space-y-1">
                <li v-for="(err, idx) in importResult.errors" :key="idx" class="text-xs text-red-700 flex items-start">
                  <span class="mr-2">第 {{ err.row }} 行:</span>
                  <span>{{ err.title || '（无标题）' }} - {{ err.message }}</span>
                </li>
              </ul>
            </div>
          </div>
...
async function handleImport() {
  ...
  try {
    ...
    importResult.value = {
      success_count: 0,
      fail_count: 1,
      errors: [{ row: 0, title: '', message: e.response?.data?.message || '导入失败' }],
      imported_ids: []
    }
  } finally {
