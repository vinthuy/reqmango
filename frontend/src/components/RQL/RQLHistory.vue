...
<template>
  <div class="absolute left-0 top-full mt-1 w-80 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
    <div class="px-3 py-2 border-b border-gray-100 flex items-center justify-between">
      <span class="text-sm font-medium text-gray-700">{{ t('rql.history') }}</span>
...
const formatTime = (timestamp: number): string => {
  ...
  if (diff < 60000) return t('rql.justNow')
  if (diff < 3600000) return t('rql.minutesAgo', { minutes: Math.floor(diff / 60000) })
  if (diff < 86400000) return t('rql.hoursAgo', { hours: Math.floor(diff / 3600000) })
  return t('rql.daysAgo', { days: Math.floor(diff / 86400000) })
}
</script>