...
function openCreate() { form = ref({ name:'', desc:'' }); showModal.value = true }
...
async function confirmDel(w:any) { if(confirm('Delete?')) { await workflowApi.deleteWorkflow(props.projectId, w.id); load() } }
function openAddTrans(w:any) { selWid.value = w.id; trans = ref({ from:0, to:0, desc:'' }); showTrans.value = true }
