<template>
  <div class="border-t border-gray-100 pt-4 mt-4">
    <div class="flex items-center justify-between mb-3">
      <h4 class="text-sm font-semibold text-gray-700">🔄 Recurrence</h4>
    </div>

    <div v-if="!rule" class="text-xs text-gray-400">
      <button @click="showForm = true" class="text-indigo-600 hover:text-indigo-700">+ Set up recurring</button>
    </div>

    <div v-else class="bg-gray-50 rounded-lg p-3 text-xs space-y-2">
      <div class="flex items-center justify-between">
        <span>Every {{ rule.interval }} {{ rule.frequency }}</span>
        <span :class="rule.is_active ? 'text-green-600' : 'text-gray-400'">{{ rule.is_active ? 'Active' : 'Paused' }}</span>
      </div>
      <div>Next: {{ formatDate(rule.next_run) }}</div>
      <div v-if="rule.end_date">Until: {{ formatDate(rule.end_date) }}</div>
      <div class="flex gap-2 pt-1">
        <button @click="edit" class="text-indigo-600 hover:text-indigo-700">Edit</button>
        <button @click="toggle" class="text-amber-600 hover:text-amber-700">{{ rule.is_active ? 'Pause' : 'Resume' }}</button>
    ...
    <div v-if="showForm" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="showForm=false">
      <div class="bg-white rounded-xl p-6 w-full max-w-sm">
        <h3 class="text-lg font-semibold mb-4">{{ rule ? 'Edit' : 'Create' }} Recurrence</h3>
        <div class="space-y-3">
          <div class="grid grid-cols-2 gap-2">
            ...
            <div>
              <label class="block text-xs font-medium mb-1">Interval</label>
              <input v-model.number="form.interval" type="number" min="1" class="w-full px-2 py-1.5 border rounded text-sm" />
            </div>
          </div>
          <div>
            <label class="block text-xs font-medium mb-1">Next Run</label>
          ...
          <div v-if="form.frequency === 'cron'">
            ...
            <input v-model="form.cron_expr" placeholder="0 9 * * 1-5" class="w-full px-2 py-1.5 border rounded text-sm" />
          </div>
        </div>
...
async function toggle() {
  ...
  await recApi.updateRecurrence(props.issueId, { is_active: !rule.value.is_active }); load()
}

async function remove() {
  if (!confirm('Remove recurrence rule?')) return
  await recApi.deleteRecurrence(props.issueId); rule.value = null
}
