...
// ============================================================
// Workspace Labels Management (Inheritance Feature)
// ============================================================
test.describe('Workspace Settings - Labels Management', () => {
  test.beforeAll(async ({ request }) => { await ensureSetup(request) })

  test('labels section loads in workspace settings', async ({ page }) => {
    await goToApp(page, `/workspace/${_wsSlug}/settings`)
    await navigateToSection(page, '标签') || await navigateToSection(page, 'Labels')
    await page.waitForTimeout(800)
    await expect(page.locator('main main')).toBeVisible({ timeout: 5000 })
  })

  test('API: create workspace-level label', async ({ request }) => {
    const res = await request.post(`${BASE_API}/workspaces/${_wsId}/settings/labels`, {
      data: { name: `E2E WS Label ${Date.now()}`, color: '#9B59B6' },
      headers: { Authorization: `Bearer ${_token}` },
    })
    expect(res.status()).toBe(201)
    const body = await res.json()
    const label = body.data || body
    expect(label.name).toContain('E2E WS Label')
  })

  test('API: list workspace-level labels', async ({ request }) => {
    const res = await request.get(`${BASE_API}/workspaces/${_wsId}/settings/labels`, {
      headers: { Authorization: `Bearer ${_token}` },
    })
    expect(res.status()).toBe(200)
    const body = await res.json()
    const labels = Array.isArray(body) ? body : (body.data || [])
    expect(labels.length).toBeGreaterThanOrEqual(0)
  })

  test('API: update workspace-level label', async ({ request }) => {
    const listRes = await request.get(`${BASE_API}/workspaces/${_wsId}/settings/labels`, {
      headers: { Authorization: `Bearer ${_token}` },
    })
    const listBody = await listRes.json()
    const labels = Array.isArray(listBody) ? listBody : (listBody.data || [])
    const label = labels.find((l: any) => l.name.includes('E2E WS Label'))
    if (!label) { test.skip(true, 'No E2E label found'); return }

    const res = await request.put(`${BASE_API}/workspaces/${_wsId}/settings/labels/${label.id}`, {
      data: { name: `${label.name} Updated`, color: '#E74C3C' },
      headers: { Authorization: `Bearer ${_token}` },
    })
    expect(res.status()).toBe(200)
  })

  test('API: delete workspace-level label', async ({ request }) => {
    const listRes = await request.get(`${BASE_API}/workspaces/${_wsId}/settings/labels`, {
      headers: { Authorization: `Bearer ${_token}` },
    })
    const listBody = await listRes.json()
    ...
    const res = await request.delete(`${BASE_API}/workspaces/${_wsId}/settings/labels/${label.id}`, {
      headers: { Authorization: `Bearer ${_token}` },
    })
    // DeleteWorkspaceLabel returns 204 No Content
    expect([200, 204]).toContain(res.status())
  })

  test('API: project labels include inherited workspace labels', async ({ request }) => {
    // First ensure a workspace-level label exists for inheritance
    await request.post(`${BASE_API}/workspaces/${_wsId}/settings/labels`, {
      data: { name: `E2E Inherit Label ${Date.now()}`, color: '#9B59B6' },
      headers: { Authorization: `Bearer ${_token}` },
    }).catch(() => {})

    // ListLabels requires workspace_id query param
    ...
    const hasInherited = labels.some((l: any) => l.is_inherited)
    expect(hasInherited).toBeTruthy()
  })
})
