...
test.describe('分析页面全功能测试', () => {
  ...
  test('TC-ANA-001: 分析页面正常加载', async ({ authedPage: page }) => {
    await expect(page.locator('text=分析').or(page.locator('text=Analytics')).or(page.locator('text=度量')).first()).toBeVisible();
  });
  ...
  test('TC-ANA-008: 空数据分析', async ({ authedPage: page }) => {
    await expect(page.locator('text=分析').or(page.locator('text=Analytics')).or(page.locator('text=度量')).or(page.locator('text=暂无数据')).first()).toBeVisible();
  });
});
