...
test.describe('旧版兼容路由', () => {
  ...
  test('TC-LGC-001: 自定义字段页面加载', async ({ authedPage: page }) => {
    ...
    await expect(page.locator('text=自定义字段, text=Custom Fields, text=字段').first()).toBeVisible();
  });
  ...
  test('TC-LGC-003: Issue 类型列表页面加载', async ({ authedPage: page }) => {
    ...
    await page.waitForTimeout(2000);
    await expect(page.locator('text=Issue 类型, text=Issue Types, text=类型').first()).toBeVisible();
  });
  ...
  test('TC-LGC-007: 旧版 Cycle 创建路由', async ({ authedPage: page }) => {
    ...
    await page.waitForTimeout(2000);
    await expect(page.locator('text=创建, text=新建, text=Cycle').first()).toBeVisible();
  });
