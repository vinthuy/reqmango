...
**目标：** 让 AI Agent 像真实成员一样被 @提及、分配工作项、自动执行操作

#### 1.1 后端：Agent 模型 [1d]

- **文件：** `backend-go/internal/model/agent.go`
- **内容：**
...
- **文件：** `backend-go/internal/service/agent_service.go`
- **内容：**
  - `ExecuteTask(agentID, taskType, context)` — 根据任务类型调用 LLM，执行操作
  - `TriageIssue(issueID)` — 自动分析 Issue，建议类型/优先级/标签/指派人
  - `AutoLabel(issueID)` — 根据内容自动打标签
  - `AutoAssign(issueID)` — 根据负载和能力自动分配
  - `SummarizeCycle(cycleID)` — 总结 Sprint 进展
  - 所有 Agent 操作写入 IssueActivity 审计日志
  - 支持 `@agent-name` 触发关键词

#### 1.3 后端：Agent 分配 API [1d]

- **文件：** `backend-go/internal/handler/agent_handler.go`
- **路由：**
  - `GET /api/v1/workspaces/:slug/agents` — 列出可用 Agent
  - `POST /api/v1/workspaces/:slug/agents/:id/dispatch` — 派发任务给 Agent
  - `POST /api/v1/workspaces/:slug/agents/:id/assign-to-issue` — 将 Agent 设为 Issue 指派人
  - `GET /api/v1/workspaces/:slug/agents/:id/activity` — 查看 Agent 操作历史

#### 1.4 后端：@mention 解析增强 [1d]

- **文件：** `backend-go/internal/service/comment_service.go`
- **内容：**
  - 扩展现有 @mention 解析逻辑，支持 `@agent-name` 模式
...
- **内容：**
  - 在指派人选择器中混入 AI Agent 列表（用不同颜色/图标区分人类和Agent）
  - Agent 头像显示为 AI 图标
  - Hover 显示 Agent 能力列表
...
- **内容：**
  - 时间线展示 Agent 的所有操作
  - 每条记录显示：操作类型、目标、结果、耗时
  - 筛选：按 Agent / 操作类型 / 时间范围

#### 1.7 AI：Agent 自动分诊增强 [2d]

- **文件：** `backend-go/internal/service/ai_service.go`（增强现有 TriageAnalyze）
- **内容：**
  - 增强分诊逻辑：自动分类 → 自动标签 → 自动分配 → 自动路由
...
**目标：** 让 Claude/Cursor/VS Code 等 AI 编程工具直接操作 ReqManPy

#### 2.1 新建 MCP Server 子项目 [2d]

- **文件：** `mcp-server/` (新建目录)
- **内容：**
  - Go module 初始化 (`mcp-server/go.mod`)
  - MCP 协议实现：Stdio 传输层
  - `main.go` 入口，注册所有 Tools
  - 连接到 ReqManPy REST API 的客户端封装

#### 2.2 Project Tools [1d]

- **文件：** `mcp-server/tools/project.go`
- **Tools：**
  - `list_projects` — 列出工作空间所有项目
  - `get_project` — 获取项目详情和统计

#### 2.3 Issue Tools [2d]

- **文件：** `mcp-server/tools/issue.go`
- **Tools：**
  - `create_issue` — 创建 Issue（title, description, priority, state, type, assignee, labels）
  - `list_issues` — 列表查询（filters, pagination）
  - `get_issue` — 获取单个 Issue 详情含评论
  - `update_issue` — 更新 Issue 状态/优先级/指派人/标签
...
- **Tools：**
  - `list_cycles` — 列出项目所有 Sprint
  - `get_cycle_progress` — 获取 Sprint 进展和燃尽数据
  - `add_issue_to_cycle` — 将 Issue 加入 Sprint
...
- **内容：**
  - HTTP + OAuth 传输模式（Plane Cloud 同款）
  - PAT Token 认证模式（CI/CD 场景）
  - 端点：`/http/mcp`
...
- **文件：** `backend-go/internal/model/git_integration.go`
- **内容：**
...
#### 3.2 GitHub Webhook 接收器 [2d]

- **文件：** `backend-go/internal/handler/git_webhook_handler.go`
- **内容：**
...
- **文件：** `backend-go/internal/service/git_service.go`
- **内容：**
...
- **内容：**
  - PR 创建时自动关联对应 Issue
  - Branch 与 Issue 的关联显示
  - PR 合并时自动关闭 Issue
  - Issue 详情展示关联 PR 状态（Open/Merged/Closed）

#### 3.5 前端：Git 集成设置页 [1.5d]

- **文件：** `frontend/src/views/GitIntegration.vue`
- **内容：**
  - OAuth 授权流程（GitHub App / GitLab App）
  - 仓库选择器（搜索 → 绑定）
  - 同步选项：PR / Commit / Branch
  - Webhook 状态和重试按钮
...
- **内容：**
  - Issue 详情右侧面板
  - 显示关联的 PR 列表（#Title, 状态, 作者, 更新时间）
  - 显示关联的 Branch 列表
  - 显示关联的 Commit 列表（最新5条）
...
- **文件：** `backend-go/internal/service/ws_hub.go`
- **内容：**
...
- **文件：** `backend-go/internal/service/collab_service.go`
- **内容：**
  - 基于 OT (Operational Transformation) 或简化 CRDT
  - 操作类型：insert, delete, replace
  - 冲突解决策略（Last-Write-Wins + 自动合并）
  - 操作广播（同一房间其他客户端）

#### 4.3 页面版本历史 [1d]

- **文件：** `backend-go/internal/model/page_version.go`
- **内容：**
  - `PageVersion` 表：`id`, `page_id`, `content`, `created_by`, `created_at`, `version_number`
  - 每次保存（非每次按键）创建版本快照
  - API：`GET /api/v1/pages/:id/versions`, `GET /api/v1/pages/:id/versions/:vid`

#### 4.4 多人 Diff 视图 API [1d]

- **文件：** `backend-go/internal/handler/page_handler.go`
- **内容：**
  - `GET /api/v1/pages/:id/versions/:vid/diff` — 返回两个版本之间的 diff
...
#### 5.1 Slack Bot 接收器 [2d]

- **文件：** `backend-go/internal/handler/slack_handler.go`
- **内容：**
  - 处理 Slack Slash Commands：`/reqman create`, `/reqman search`, `/reqman status`
  - 处理 Slack Events API（消息事件，消息操作）
  - 签名验证

#### 5.2 Slack → Issue [1.5d]

- **文件：** `backend-go/internal/service/slack_service.go`
- **内容：**
  - `CreateIssueFromThread(channelID, threadTS)` — 将 Slack 线程转为 Issue
  - 消息线程映射表存储
  - 支持双向同步：Issue 评论 ↔ Slack 回复
...
- **文件：** `backend-go/internal/service/ai_service.go`（新增 Chart 方法）
- **内容：**
  - 自然语言 → 图表查询 → 结构化图表定义
  - 支持的图表类型：bar, line, pie, doughnut, burndown, cumulative_flow
  - 返回格式：`{ type, title, labels, datasets, options }`

#### 6.2 前端：AI Chart 渲染器 [2d]

- **文件：** `frontend/src/components/AIChart.vue`
- **内容：**
  - 基于 Chart.js / ECharts 的动态图表渲染
  - 类型切换（柱状图 ↔ 饼图 ↔ 折线图）
  - 导出 PNG / SVG
...
- **文件：** `backend-go/internal/handler/email_intake_handler.go`
- **内容：**
  - 方案 A：Mailgun/Postmark Webhook 接收
  - 方案 B：IMAP 定时拉取
  - 邮件解析：From→提交人, Subject→标题, Body→描述, 附件→Issue 附件
  - 去重检测（Message-ID）

#### 7.2 API Intake 增强 [1d]

- **文件：** `backend-go/internal/handler/intake_handler.go`（增强现有）
- **内容：**
  - API Key 认证模式（用于外部系统集成）
  - 自定义字段填充支持
  - 回调 URL 配置（创建成功后通知外部系统）
...
- **内容：**
  - 按来源/关键词/类型自动路由
  - 条件：if 来源=Email ∧ 主题含"Bug" → 项目=工程部, 类型=Bug
  - 规则优先级排序
...
- **文件：** `backend-go/internal/handler/workspace_handler.go`
- **内容：**
  - `GET /api/v1/workspaces/:slug/issues` — 跨项目 Issue 聚合
  - 支持按项目/状态/指派人/类型过滤
  - 分页支持
...
- **内容：**
  ...
  - `redis-deployment.yaml` + `redis-service.yaml`（用于 Session 和协作）
  - `ingress.yaml`
  - `configmap.yaml` + `secret.yaml`

#### 10.3 水平扩展 [2d]

- **文件：** `backend-go/internal/config/`（增强）+ Helm values
- **内容：**
  - Backend 无状态化确认
  - Redis Session 存储（替代内存）
  - HPA 配置（CPU/Memory 自动扩缩）
  - 就绪探针 / 存活探针
...
- **内容：**
  - 前置条件（K8s 1.25+, Helm 3.12+, kubectl）
  - 快速开始：`helm install reqmanpy ./reqmanpy -f values-prod.yaml`
...
- **内容：**
  ...
  - `reqman/__init__.py` — Client 类，支持 API Key 和 JWT 认证
  ...
  - `reqman/ai.py` — AI 端点（chat, search, analyze）
  - 类型标注 + Docstring
...
- **文件：** `backend-go/internal/service/import_service.go`
- **内容：**
  - Jira REST API v3 调用
...
- **内容：**
  - 步骤1：选择源（Jira Cloud / Jira DC / Linear / Asana / CSV）
  - 步骤2：连接配置（URL + Token）
  - 步骤3：数据映射预览（字段对应表）
  - 步骤4：确认并执行（进度条 + 实时日志）
  - 步骤5：结果报告（成功/失败/跳过数量）
...
- **文件：** `backend-go/internal/model/vote.go`
- **内容：**
  - `Vote` 表：`id`, `issue_id`, `user_id`, `vote` (+1/-1), `created_at`
  - 唯一约束：(issue_id, user_id)
- **Handler：**
  - `POST /api/v1/issues/:id/vote` — 投票（body: `{vote: +1|-1}`）
  - `DELETE /api/v1/issues/:id/vote` — 取消投票
...
| **P2** | 6. AI 交互式图表 | 4 | 6.0d | 🟢 |
| **P2** | 7. 多渠道 Intake | 4 | 5.5d | 🟢 |
| **P2** | 8. 看板 Swimlane | 2 | 3.5d | 🟢 |
| **P2** | 9. 工作空间跨项目视图 | 3 | 4.5d | 🟢 |
...
> **说明：** 以上为单人力估算。2-3 人团队可将时间缩至 6-8 周。
