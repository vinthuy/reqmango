...
�?reqmango 后端�?Python/FastAPI 重构�?Golang，第一期聚焦核�?CRUD �?Issue 管理，逐步替换�?
## 技术栈

| �?| 选型 | 理由 |
|---|---|---|
| HTTP 框架 | **Gin** | 最流行，类 FastAPI 风格，生态丰�?|
| ORM | **GORM** | Go 最流行 ORM，自动迁移，关联加载 |
| 数据�?| **PostgreSQL** | 生产�?|
| 认证 | **JWT** | Access Token + Refresh Token |
| 配置 | **Viper** | 读取环境变量 |
| 迁移 | **GORM AutoMigrate** | 开发阶段自动建�?|

## 项目结构（经典分层）

```
backend/
├── cmd/server/main.go          # 入口：初始化 DB、注册路由、启�?├── internal/
�?  ├── config/                 # Viper 配置加载
�?  �?  └── config.go
�?  ├── model/                  # GORM 模型
�?  �?  ├── base.go             # 公共字段（ID, Timestamps, SoftDelete�?�?  �?  ├── user.go
�?  �?  ├── workspace.go
�?  �?  ├── project.go
�?  �?  ├── state.go
�?  �?  ├── label.go
�?  �?  └── issue.go
�?  ├── dto/                    # 请求/响应结构�?�?  ├── repository/             # 数据访问�?�?  ├── service/                # 业务逻辑�?�?  ├── handler/                # HTTP 层（Gin handlers�?�?  ├── middleware/              # JWT Auth、CORS、Recovery
�?  └── router/                 # 路由注册
├── config/config.yaml          # 默认配置
└── go.mod
```

分层职责�?- **handler** �?绑定请求参数 �?�?service �?�?HTTP 响应
- **service** �?业务逻辑，跨 repository 调用
- **repository** �?封装 GORM 查询，单一数据源操�?- **model** �?表结构定义，GORM 标签
...
### Auth（公开�?- `POST /api/v1/auth/register` �?注册
- `POST /api/v1/auth/login` �?登录，返�?JWT
- `GET  /api/v1/auth/me` 🔒 �?当前用户信息

### Workspace 🔒
- `GET    /api/v1/workspaces` �?用户的工作空间列�?- `POST   /api/v1/workspaces` �?创建
- `GET    /api/v1/workspaces/:id` �?详情
- `PUT    /api/v1/workspaces/:id` �?更新
- `DELETE /api/v1/workspaces/:id` �?软删�?
### Project 🔒
...
### State 🔒
- `GET    /api/v1/projects/:pid/states` �?状态列�?- `POST   /api/v1/projects/:pid/states` �?创建
...
### Issue 🔒
- `GET    /api/v1/projects/:pid/issues` �?Issue 列表（支�??state_id=&priority=&search= 过滤�?- `POST   /api/v1/projects/:pid/issues` �?创建（可�?assignees、labels�?- `GET    /api/v1/issues/:id` �?详情（含关联数据�?- `PUT    /api/v1/issues/:id` �?更新（含状态变更、分配人/label 调整�?- `DELETE /api/v1/issues/:id` �?软删�?
### 响应格式
```json
// 单条
{ "data": {...}, "message": "ok" }
...
| 步骤 | 内容 | 依赖 |
|---|---|---|
| 1 | 项目脚手架：go mod、目录、Gin 启动、配置、GORM 连接 | �?|
| 2 | User 模型 + Auth：注�?登录、JWT 中间�?| 1 |
| 3 | Workspace CRUD（含 WorkspaceMember�?| 2 |
| 4 | Project CRUD（含 ProjectMember�?| 3 |
| 5 | State CRUD | 4 |
| 6 | Label CRUD | 4 |
| 7 | Issue CRUD + 关联�?+ 过滤查询 + 状态变�?| 4,5,6 |

每步完成�?curl 验证。路由保持与 FastAPI 兼容，前端无需改动�?
## 不包含（第一期）
